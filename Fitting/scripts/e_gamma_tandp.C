...
Double_t Get_Fake_Rate(TString variable, Double_t x1 ,Double_t x2 )
{
	...
	TH1F* h_cut_on_probe = (TH1F*) Mass( variable,  x1 , x2 , "Cut" , 1 );
	//produce histogram with cut on probe and also electron ID
	TH1F* h_cut_on_probe_electron_ID = (TH1F*) Mass( variable , x1, x2, "Cut", 0 );
...
void Draw_phase_space( TString variable , Int_t nbins , Double_t xMin , Double_t xMax )
{
	...
	h_phase_space->GetYaxis()->SetTitle("Efficiency");
	//const Char_t* title =  ;
	h_phase_space->SetTitle( variable.Data());
...
void e_gamma_tandp()
{
	...
	Draw_phase_space( "p_T" , 10 , 0 , 800 );
	Draw_phase_space("eta", 10, -5, 5 );
